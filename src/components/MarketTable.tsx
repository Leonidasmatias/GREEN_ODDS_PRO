  ...
  return (
    <div className="overflow-x-auto scrollbar-none">
      <table className="w-full min-w-[1160px] text-left">
        <thead><tr className="border-b border-line text-[9px] uppercase tracking-[.15em] text-zinc-600"><th className="px-4 py-4">Jogo / Entrada</th><th>Odd</th><th>Implicita</th><th>Estimativa</th><th>Edge</th><th>EV</th><th>Risco</th><th className="min-w-28">Score</th><th>Power</th><th>Classificacao</th><th>Sinal</th></tr></thead>
        <tbody>{visibleItems.length ? visibleItems.map((item) => {
          ...
          return <tr key={item.id} className={`border-b border-line/60 text-xs transition hover:bg-white/[.025] ${positiveEV ? "bg-neon/[.025]" : ""}`}>
            ...
            <td className={item.edge > 0 ? "font-bold text-neon" : "text-zinc-500"}>{item.edge > 0 ? "+" : ""}{pct(item.edge)}</td>
            <td className={positiveEV ? "font-black text-neon" : "font-bold text-zinc-500"}>{positiveEV ? "+" : ""}{pct(item.expectedValue)}</td>
        ...
        }) : <tr><td colSpan={11} className="px-4 py-14 text-center text-sm text-zinc-600">Nenhuma partida encontrada</td></tr>}</tbody>
      </table>
    </div>
  );
}
