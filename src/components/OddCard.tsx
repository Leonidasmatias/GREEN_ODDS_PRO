    ...
    <article className={`group rounded-2xl border p-5 transition hover:-translate-y-0.5 ${positiveEV ? "border-neon/25 bg-gradient-to-br from-neon/[.08] to-panel shadow-glow" : "border-line bg-panel"}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="mb-2 flex items-center gap-2 text-[9px] font-bold uppercase tracking-wider text-zinc-600"><Clock3 size={11}/> Provider ativo</div>
      ...
      <div className="my-5 grid grid-cols-3 gap-2">
        ...
        <div><p className="label">EV</p><strong className={`mt-1 block text-xl ${positiveEV ? "text-neon" : "text-zinc-400"}`}>{percent(opportunity.expectedValue)}</strong></div>
      ...
      <div className="mt-4 flex items-center justify-between"><RiskBadge risk={opportunity.risk}/><span className="text-[10px] text-zinc-600">Confianca {opportunity.confidence}%</span></div>
    </article>
  );
}
