...
export function ValueOpportunityTable({ items, empty = "Nenhuma entrada green validada no momento." }: { items: ValueOpportunity[]; empty?: string }) {
  return <div className="overflow-x-auto scrollbar-none">
    <table className="w-full min-w-[1220px] text-left text-xs">
      <thead><tr className="border-b border-line text-[9px] uppercase tracking-[.15em] text-zinc-600"><th className="px-4 py-4">Jogo / Mercado</th><th>Odd</th><th>Implícita</th><th>Margem</th><th>Fair odd</th><th>Prob. estimada</th><th>Edge</th><th>EV</th><th>Confiança</th><th>Risco</th><th>Score</th><th>Classificação</th></tr></thead>
      <tbody>{items.length ? items.map((item) => <tr key={item.id} className="border-b border-line/60 transition hover:bg-white/[.025]">
        <td className="px-4 py-4"><b className="block text-white">{item.game}</b><span className="mt-1 block text-[10px] text-zinc-500">{item.market} · {item.selection}</span></td>
      ...
      </tr>) : <tr><td colSpan={12} className="px-4 py-14 text-center text-sm text-zinc-600">{empty}</td></tr>}</tbody>
    </table>
  </div>;
}

export function ValueAuditSummary({ analyzed, approved, rejected, watch, insufficientRealData, provider, rejectionReasons }: { analyzed: number; approved: number; rejected: number; watch: number; insufficientRealData: number; provider: string; rejectionReasons?: Record<string, number> }) {
  const rows = [
    ["Provider", provider],
    ...
    ["Histórico insuficiente", insufficientRealData.toString()],
  ];
  const reasons = Object.entries(rejectionReasons ?? {}).sort((a, b) => b[1] - a[1]).slice(0, 4);
  return <div className="space-y-3">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">{rows.map(([label, value]) => <div key={label} className="card p-4"><p className="label">{label}</p><strong className="mt-3 block text-lg text-white">{value}</strong></div>)}</div>
