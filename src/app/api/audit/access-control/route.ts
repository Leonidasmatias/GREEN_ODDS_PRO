...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    activePlan: planCode,
    checks,
    bypass: checks.some((item) => item.feature && !item.allowed && ["commandCenter", "performanceCenter"].includes(item.feature)) ? "PREMIUM_FEATURE_BLOCKED_FOR_CURRENT_PLAN" : "NO_BYPASS_DETECTED",
    recentDenied,
